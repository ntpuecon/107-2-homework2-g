...
summary(model1)
```
由表可知:在其他條件不變下，當媽媽受教育年數每增加一年，孩子數會下降0.09個
...
summary(model2)
```
由表可知，frsthalf之係數顯著不為0，所以和educ具有關聯性
且F-statistic=(-7.554)^2 > 10 根據經驗法則，frsthalf是一個合理的強工具變數。
...
model5_tests
```
由表可知Hausman統計量為1.936，沒有大於自由度為6的卡方，在5%的顯著水準下不拒絕Ho，用OLS或TSLS都可以，係數值都很相近。
...
$E(y_i|\tilde{x}_i,w_i)=\tilde{x}_i\beta^{\star}+\gamma w_i+y_{-x,-w,i}如果要有一致性的話需要\tilde{x}_i\perp\epsilon_i才會有$
...
$用\tilde{w}_i替代w_i做為控制變數，\tilde{w}_i與Y_{-x_i}有關且\tilde{w}_i與x_i有關$
